package com.lushihao.myutils.math;

import java.math.BigDecimal;

public final class LSHMathUtils {
    ...
    public static String add(String v1, String v2) {
        ...
        if (!isNumber(v1)) {
            throw new IllegalArgumentException("参数1不是数字");
        }
        if (!isNumber(v2)) {
            throw new IllegalArgumentException("参数2不是数字");
        }
    ...
    public static String sum(String v1, String... valList) {
        ...
        if (!isNumber(valList)) {
            throw new IllegalArgumentException("参数2中包含不是数字的数据");
        }
        BigDecimal b1 = new BigDecimal(v1.trim());
    ...
    public static String avg(String... valList) {
        if (!isNumber(valList)) {
            throw new IllegalArgumentException("参数中包含不是数字的数据");
        }
        if (null != valList && valList.length != 0) {
     ...
     * @return
     */
    public static BigDecimal getWithScale(BigDecimal bigDecimal, Integer scale, Integer type) {
     ...
     * 判断字符串是否为数字(不依赖第三方)
     */
    private static boolean isNumber(String v1) {
        return v1.matches("(\\d*\\.?\\d*)");
    }

    /**
     * 判断字符串是否为数字(不依赖第三方)
     */
    private static boolean isNumber(String... valList) {
        for (String val : valList)
            if (!isNumber(val))
                return false;
        return true;
